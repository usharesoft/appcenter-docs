...
.. note:: ``uforge-sync`` is based on rsync to handle synchronization, which is not powerful at synchronizing BTRFS subvolumes. If you plan you synchronize ``snapshots`` directories from a BTRFS partition, it is better to exclude these directories from the scan and use another tool to handle synchronization, based on `btrfs send` and `btrfs receive`. 
